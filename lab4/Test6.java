class Test6 {
  public static void main(String[] args) {
    ...
    we.expect(
        "Probably.just(17).<Integer>apply(justIncr)",
        Probably.just(17).apply(justIncr).toString(),
        "<18>");
    we.expect(
        "Probably.<Integer>none().<Integer>apply(justIncr)",
        Probably.<Integer>none().apply(justIncr).toString(),
        "<>");
    we.expect(
        "Probably.just(17).<Integer>apply(noIncr)",
        Probably.just(17).apply(noIncr).toString(),
        "<>");
    we.expect(
        "Probably.<Integer>none().<Integer>apply(noIncr)",
        Probably.<Integer>none().apply(noIncr).toString(),
        "<>");

    we.expect(
        "Probably.just(\"string\").<Integer>apply(justLength)",
        Probably.just("string").apply(justLength).toString(),
        "<6>");
    we.expect(
        "Probably.<String>none().<Integer>apply(justLength)",
        Probably.<String>none().apply(justLength).toString(),
        "<>");
    we.expect(
        "Probably.just(\"string\").<Integer>apply(noLength)",
        Probably.just("string").apply(noLength).toString(),
        "<>");
    we.expect(
        "Probably.<String>none().<Integer>apply(noLength)",
