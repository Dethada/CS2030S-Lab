...
public class Test4 {
  public static void main(String[] args) {
    ...
    we.expect(
        ...
        superNat.toString(),
        "[1, 2, ?, 3, ?, ?, 4, ?, ?, ?, 5, ?, ?, ?, ?]");
    we.expect("... and we check the number of evaluation on dupl", _dupl[0], 5);

    we.expect("... and there are 0 evaluation on copy", _copy[0], 0);
    int[] _dbl = new int[] {0};
    ...
    we.expect(
        ...
        superEven.toString(),
        "[?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?]");
    we.expect("... and we check the number of evaluation on dbl", _dbl[0], 0);
    we.expect("... and there are 0 additional evaluation on dupl", _dupl[0], 5);
    we.expect("... and there are still 0 evaluation on copy", _copy[0], 0);

    superEven.get(12);
    we.expect(
        "Retrieving index 12 on super even number causes 1 evaluation on dbl ...", _dbl[0], 1);
    we.expect("... and causes 0 additional evaluations on dupl", _dupl[0], 5);
    we.expect("... and causes 2 evaluations on copy", _copy[0], 2);
    ...
    we.expect(
        ...
        nestNat2.toString(),
        "[?, ?, [3, ?, ?], ?, ?]");
    we.expect("... and we check the number of evaluation on dupl", _dupl[0], 6);
