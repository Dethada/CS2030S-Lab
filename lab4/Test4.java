class Test4 {
  public static void main(String[] args) {
    ...
    we.expect(
        "Probably.just(4).transform(new Incr())",
        Probably.just(4).transform(new Incr()).toString(),
        "<5>");
    we.expect(
        "Probably.just(4).transform(new Incr()).transform(new Incr())",
    ...
    we.expect(
        ...
        "<6>");
    we.expect(
        "Probably.just(\"string\").transform(new Length()).transform(new Incr())",
        Probably.just("string").transform(new Length()).transform(new Incr()).toString(),
        "<7>");

    we.expect(
        "Probably.<Integer>none().transform(new Incr())",
        Probably.<Integer>none().transform(new Incr()).toString(),
        "<>");
    we.expect(
        "Probably.<String>none().transform(new Length())",
        Probably.<String>none().transform(new Length()).toString(),
        "<>");
    we.expect(
        "Probably.<String>just(null).transform(new Length()).transform(new Incr())",
