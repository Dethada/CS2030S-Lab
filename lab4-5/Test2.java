...
public class Test2 {
  public static void main(final String[] args) {
    ...
    we.expectPrint("Actually.<Integer>ok(9).finish(print)", "9", baos, old);

    baos = new ByteArrayOutputStream();
    ps = new PrintStream(baos);
    System.setOut(ps);
    cs2030s.fp.Actually.<Integer>err(new Exception("Err")).finish(print);
    we.expectPrint("Actually.<Integer>err(new Exception(\"Err\")).finish(print)", "", baos, old);
    ...
    we.expect(
        ...
        cs2030s.fp.Actually.<Number>err(new ArithmeticException("div by 0"))
            .except(zero)
            .toString(),
        "0");
